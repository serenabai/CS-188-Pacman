    ...
    def train(self, dataset):
        ...
        while accuracy < 0.975:
            for x,y in dataset.iterate_once(300):
                ...
                self.w1.update(grad_wrt_w1, -learn_rate)
                self.b1.update(grad_wrt_b1, -learn_rate)
                self.w2.update(grad_wrt_w2, -learn_rate)
                self.b2.update(grad_wrt_b2, -learn_rate)
                self.w3.update(grad_wrt_w3, -learn_rate)
                self.b3.update(grad_wrt_b3, -learn_rate)
...
class LanguageIDModel(object):
    ...
    def __init__(self):
        ...
        self.w_final = nn.Parameter(300, 5)
    ...
    def run(self, xs):
        ...
        """
        "*** YOUR CODE HERE ***"
        f = nn.ReLU(nn.AddBias(nn.Linear(xs[0], self.w1), self.b1))
        for letter in xs[1:]:
            f = nn.ReLU(nn.AddBias(nn.Add(nn.Linear(letter, self.w1), nn.Linear(f, self.w_hidden)), self.b_hidden))# z = nn.Add(nn.Linear(x, W), nn.Linear(h, W_hidden))

        final = nn.AddBias(nn.Linear(f, self.w_final), self.b_final)
        return final
    ...
    def train(self, dataset):
        ...
        while accuracy < 0.89:
            for x,y in dataset.iterate_once(100):
                grad_wrt_w1, grad_wrt_b1, grad_wrt_w2, grad_wrt_b2, grad_wrt_w3, grad_wrt_b3  = nn.gradients(
                self.get_loss(x, y), [self.w1, self.b1, self.w_hidden, self.b_hidden, self.w_final, self.b_final])
                self.w1.update(grad_wrt_w1, -learn_rate)
                self.b1.update(grad_wrt_b1, -learn_rate)
                self.w_hidden.update(grad_wrt_w2, -learn_rate)
                self.b_hidden.update(grad_wrt_b2, -learn_rate)
                self.w_final.update(grad_wrt_w3, -learn_rate)
                self.b_final.update(grad_wrt_b3, -learn_rate)
