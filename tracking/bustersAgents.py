    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "* YOUR CODE HERE *"
        # compute most likely position of each ghost tht has not yet been captured
        dist = float('inf')
        temp_dist = 0
        closest_ghost = None
        for ghost in livingGhostPositionDistributions:
            ghost_pos = ghost.argMax()
            temp_dist = self.distancer.getDistance(pacmanPosition, ghost_pos)
            if temp_dist < dist:
                dist = temp_dist
        ...
        for a in legal:
            ...
            temp_distance = self.distancer.getDistance(successorPosition, closest_ghost)
