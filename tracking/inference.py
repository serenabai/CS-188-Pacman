...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict[PAC] = locations
    variableDomainsDict[GHOST0] = locations
    variableDomainsDict[GHOST1] = locations
    observations = []
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPos in self.allPositions:
            ...
            for newPos in newPosDist:
                new_beliefs[newPos] += self.beliefs[oldPos] * newPosDist[newPos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        legal_p = self.legalPositions

        num_positions = len(legal_p)

        for i in range(num_particles):
            self.particles.append(legal_p[i % num_positions])
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        new_particles = DiscreteDistribution()

        for p in self.particles:
            probability = self.getObservationProb(observation, pacmanPosition, p, jailPosition)

            if p in new_particles:
                new_particles[p] = new_particles[p] + probability
            else:
                new_particles[p] = probability
        
        if new_particles.total() == 0:
            self.initializeUniformly(gameState)
        else:
            final_particles = []
            for i in range(self.numParticles):
                final_particles.append(new_particles.sample())
