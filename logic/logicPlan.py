...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    A_notB = premise & ~conclusion

    if findModel(A_notB) == False:
        return True
    else:
        return False
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for x, y in non_outer_wall_coords:
        one_square.append(PropSymbolExpr(pacman_str, x, y, time = t))
    
    one_square_sentences = exactlyOne(one_square)
    one_move_sentence = exactlyOne([PropSymbolExpr('North', time=t), PropSymbolExpr('South', time=t), PropSymbolExpr('East', time=t), PropSymbolExpr('West', time=t)])
    pacphysics_sentences.append(one_square_sentences)
    pacphysics_sentences.append(one_move_sentence)
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    KB.append(pacphysicsAxioms(t+1, all_coords, non_outer_wall_coords, walls_grid))
    ...
    # In model1, Pacman is at (x1, y1)(x1,y1) at time t = 1t=1 given x0_y0, action0, action1. 
    # This model proves that its possible that Pacman there. Notably, if model1 is False, we know Pacman is guaranteed to NOT be there.
    model1 = conjoin(KB) & PropSymbolExpr(pacman_str, x1, y1, time = t+1) #>> conjoin(PropSymbolExpr(pacman_str, x0, y0, time = t), PropSymbolExpr(action0, time = t), PropSymbolExpr(action1, time = t))
    #In model2, Pacman is NOT at (x1, y1)(x1,y1) at time t = 1t=1 given x0_y0, action0, action1. 
    #This model proves that its possible that Pacman is not there. Notably, if model2 is False, we know Pacman is guaranteed to be there.
    model2 = conjoin(KB) & ~PropSymbolExpr(pacman_str, x1, y1, time = t+1) #>> conjoin(PropSymbolExpr(pacman_str, x0, y0, time = t), PropSymbolExpr(action0, time = t), PropSymbolExpr(action1, time = t))

    # KB.append(model1)
    # KB.append(model2)

    model1 = findModel(model1)
    model2 = findModel(model2)
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        for i in non_wall_coords:
            pl.append(PropSymbolExpr(pacman_str, i[0], i[1], time=t))
        KB.append(exactlyOne(pl))
        ...
        for n in non_wall_coords:
            KB.append(pacmanSuccessorAxiomSingle(n[0], n[1], t+1, walls_grid))
    "*** END YOUR CODE HERE ***"
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        for n in non_wall_coords:
            KB.append(pacmanSuccessorAxiomSingle(n[0], n[1], t+1, walls))   

        fl = []
        for x, y in food:
            fl.append(~PropSymbolExpr(food_str, x, y, time=t))
        food_goal = conjoin(fl)
        model = findModel(conjoin(KB) & food_goal)
        if model != False:
            return extractActionSequence(model, actions)
        
        for fx, fy in food:
            a = (PropSymbolExpr(food_str, fx, fy, time=t) & PropSymbolExpr(pacman_str, fx, fy, time=t))
            b = ~PropSymbolExpr(food_str, fx, fy, time=t+1)
            c = ~PropSymbolExpr(food_str, fx, fy, time=t)
            KB.append(a % b | c)
    "*** END YOUR CODE HERE ***"
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        #Add pacphysics, action, and percept information to KB
        #(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], 
        # walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None)
...
def slam(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for x, y in non_outer_wall_coords:
            wall_model1 = findModel(conjoin(KB) & PropSymbolExpr(wall_str, x, y))
            wall_model2 = findModel(conjoin(KB) & ~PropSymbolExpr(wall_str, x, y))

            if wall_model1 == False:
                KB.append(~PropSymbolExpr(wall_str, x, y))
            if wall_model2 == False:
                KB.append(PropSymbolExpr(wall, x, y))
            if wall_model2:
                known_map[x][y] = 0
                if wall_model1:
                    known_map[x][y] = -1
